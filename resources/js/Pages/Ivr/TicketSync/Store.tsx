...
function TicketSyncStore({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/ticket-sync/store?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="TicketSync Store" />
      <h1 className="mb-4 text-2xl font-bold">TicketSync – Store</h1>
...
TicketSyncStore.layout = authenticatedLayout
export default TicketSyncStore
