...
function TicketSyncSync({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/ticket-sync/sync?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="TicketSync Sync" />
      <h1 className="mb-4 text-2xl font-bold">TicketSync – Sync</h1>
...
TicketSyncSync.layout = authenticatedLayout
export default TicketSyncSync
