...
function RoleAccessStore({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/role-access/store?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="RoleAccess Store" />
      <h1 className="mb-4 text-2xl font-bold">RoleAccess – Store</h1>
...
RoleAccessStore.layout = authenticatedLayout
