...
function QueueManagementIndex({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/queue-management/index?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="QueueManagement Index" />
      <h1 className="mb-4 text-2xl font-bold">QueueManagement – Index</h1>
      ...
      <div className="mb-4 rounded border border-dashed border-gray-400 p-3 text-sm">Legacy panel QueueManagement v2</div>
      ...
      <Link href="/ivr" className="text-indigo-600 underline">Back to IVR hub</Link>
    </div>
  )
}

QueueManagementIndex.layout = authenticatedLayout
export default QueueManagementIndex
