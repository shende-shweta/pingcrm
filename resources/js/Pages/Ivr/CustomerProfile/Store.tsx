...
function CustomerProfileStore({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/customer-profile/store?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="CustomerProfile Store" />
      <h1 className="mb-4 text-2xl font-bold">CustomerProfile – Store</h1>
...
CustomerProfileStore.layout = authenticatedLayout
