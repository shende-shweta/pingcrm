...
function MediaServerSync({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/media-server/sync?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="MediaServer Sync" />
      <h1 className="mb-4 text-2xl font-bold">MediaServer – Sync</h1>
...
MediaServerSync.layout = authenticatedLayout
export default MediaServerSync
