...
function MediaServerMonitor({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/media-server/monitor?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="MediaServer Monitor" />
      <h1 className="mb-4 text-2xl font-bold">MediaServer – Monitor</h1>
      ...
      <div className="mb-4 rounded border border-dashed border-gray-400 p-3 text-sm">Legacy panel MediaServer v3</div>
...
MediaServerMonitor.layout = authenticatedLayout
export default MediaServerMonitor
