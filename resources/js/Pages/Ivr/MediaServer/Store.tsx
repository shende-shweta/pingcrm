...
function MediaServerStore({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/media-server/store?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="MediaServer Store" />
      <h1 className="mb-4 text-2xl font-bold">MediaServer – Store</h1>
...
MediaServerStore.layout = authenticatedLayout
export default MediaServerStore
