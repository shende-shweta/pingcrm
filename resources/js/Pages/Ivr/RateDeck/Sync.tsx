...
function RateDeckSync({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/rate-deck/sync?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="RateDeck Sync" />
      <h1 className="mb-4 text-2xl font-bold">RateDeck – Sync</h1>
...
RateDeckSync.layout = authenticatedLayout
export default RateDeckSync
