...
function RateDeckStore({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/rate-deck/store?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="RateDeck Store" />
      <h1 className="mb-4 text-2xl font-bold">RateDeck – Store</h1>
...
RateDeckStore.layout = authenticatedLayout
export default RateDeckStore
