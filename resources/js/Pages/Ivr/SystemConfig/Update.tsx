...
function SystemConfigUpdate({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/system-config/update?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="SystemConfig Update" />
      <h1 className="mb-4 text-2xl font-bold">SystemConfig – Update</h1>
...
SystemConfigUpdate.layout = authenticatedLayout
