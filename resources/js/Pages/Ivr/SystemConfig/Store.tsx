...
function SystemConfigStore({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/system-config/store?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="SystemConfig Store" />
      <h1 className="mb-4 text-2xl font-bold">SystemConfig – Store</h1>
...
SystemConfigStore.layout = authenticatedLayout
