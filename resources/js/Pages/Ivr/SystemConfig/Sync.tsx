...
function SystemConfigSync({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/system-config/sync?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="SystemConfig Sync" />
      <h1 className="mb-4 text-2xl font-bold">SystemConfig – Sync</h1>
      ...
      <div className="mb-4 rounded border border-dashed border-gray-400 p-3 text-sm">Legacy panel SystemConfig v3</div>
...
SystemConfigSync.layout = authenticatedLayout
export default SystemConfigSync
