...
function CrmBridgeStore({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/crm-bridge/store?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="CrmBridge Store" />
      <h1 className="mb-4 text-2xl font-bold">CrmBridge – Store</h1>
...
CrmBridgeStore.layout = authenticatedLayout
export default CrmBridgeStore
