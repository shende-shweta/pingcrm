...
function WebhookDispatcherIndex({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/webhook-dispatcher/index?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="WebhookDispatcher Index" />
      <h1 className="mb-4 text-2xl font-bold">WebhookDispatcher – Index</h1>
      ...
      <div className="mb-4 rounded border border-dashed border-gray-400 p-3 text-sm">Legacy panel WebhookDispatcher v3</div>
...
WebhookDispatcherIndex.layout = authenticatedLayout
export default WebhookDispatcherIndex
