...
function WebhookDispatcherStore({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/webhook-dispatcher/store?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="WebhookDispatcher Store" />
      <h1 className="mb-4 text-2xl font-bold">WebhookDispatcher – Store</h1>
...
WebhookDispatcherStore.layout = authenticatedLayout
export default WebhookDispatcherStore
