...
function WebhookDispatcherSync({ rows = [], filters = {}, legacyMeta = {} }: { rows?: Row[]; filters?: Record<string, unknown>; legacyMeta?: Record<string, unknown> }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetch('/ivr-legacy/webhook-dispatcher/sync?q=' + search)
        .then(r => r.json())
  ...
  return (
    <div style={{ padding: 12 }}>
      <Head title="WebhookDispatcher Sync" />
      <h1 className="mb-4 text-2xl font-bold">WebhookDispatcher – Sync</h1>
...
WebhookDispatcherSync.layout = authenticatedLayout
export default WebhookDispatcherSync
