...
class CallAnalyticsSyncController extends Controller
{
    ...
    public function handleSync(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "CallAnalytics", "action" => "Sync"]);
        }

        return Inertia::render("Ivr/CallAnalytics/Sync", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2089, "idx" => 55],
        ]);
    }
