...
class BusinessHoursIndexController extends Controller
{
    ...
    public function handleIndex(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "BusinessHours", "action" => "Index"]);
        }

        return Inertia::render("Ivr/BusinessHours/Index", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2063, "idx" => 35],
        ]);
    }
