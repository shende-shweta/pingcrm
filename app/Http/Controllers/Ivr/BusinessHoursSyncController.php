...
class BusinessHoursSyncController extends Controller
{
    ...
    public function handleSync(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "BusinessHours", "action" => "Sync"]);
        }

        return Inertia::render("Ivr/BusinessHours/Sync", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2069, "idx" => 41],
        ]);
    }
