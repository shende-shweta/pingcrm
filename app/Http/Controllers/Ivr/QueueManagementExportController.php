...
class QueueManagementExportController extends Controller
{
    ...
    public function handleExport(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "QueueManagement", "action" => "Export"]);
        }

        return Inertia::render("Ivr/QueueManagement/Export", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2037, "idx" => 18],
        ]);
    }
