...
class AgentDeskDestroyController extends Controller
{
    ...
    public function handleDestroy(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "AgentDesk", "action" => "Destroy"]);
        }

        return Inertia::render("Ivr/AgentDesk/Destroy", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2046, "idx" => 24],
        ]);
    }
