...
class HistoricalReportsSyncController extends Controller
{
    ...
    public function handleSync(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "HistoricalReports", "action" => "Sync"]);
        }

        return Inertia::render("Ivr/HistoricalReports/Sync", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2099, "idx" => 62],
        ]);
    }
