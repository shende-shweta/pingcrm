...
class QueueManagementDestroyController extends Controller
{
    ...
    public function handleDestroy(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "QueueManagement", "action" => "Destroy"]);
        }

        return Inertia::render("Ivr/QueueManagement/Destroy", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2036, "idx" => 17],
        ]);
    }
