...
class CallAnalyticsExportController extends Controller
{
    ...
    public function handleExport(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "CallAnalytics", "action" => "Export"]);
        }

        return Inertia::render("Ivr/CallAnalytics/Export", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2087, "idx" => 53],
        ]);
    }
