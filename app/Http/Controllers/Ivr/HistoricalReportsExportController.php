...
class HistoricalReportsExportController extends Controller
{
    ...
    public function handleExport(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "HistoricalReports", "action" => "Export"]);
        }

        return Inertia::render("Ivr/HistoricalReports/Export", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2097, "idx" => 60],
        ]);
    }
