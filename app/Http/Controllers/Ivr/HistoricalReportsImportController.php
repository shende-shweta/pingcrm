...
class HistoricalReportsImportController extends Controller
{
    ...
    public function handleImport(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "HistoricalReports", "action" => "Import"]);
        }

        return Inertia::render("Ivr/HistoricalReports/Import", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2098, "idx" => 61],
        ]);
    }
