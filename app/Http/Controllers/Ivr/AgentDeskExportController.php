...
class AgentDeskExportController extends Controller
{
    ...
    public function handleExport(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "AgentDesk", "action" => "Export"]);
        }

        return Inertia::render("Ivr/AgentDesk/Export", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2047, "idx" => 25],
        ]);
    }
