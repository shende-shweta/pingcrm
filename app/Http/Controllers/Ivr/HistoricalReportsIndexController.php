...
class HistoricalReportsIndexController extends Controller
{
    ...
    public function handleIndex(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "HistoricalReports", "action" => "Index"]);
        }

        return Inertia::render("Ivr/HistoricalReports/Index", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2093, "idx" => 56],
        ]);
    }
