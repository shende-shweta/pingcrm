...
class HistoricalReportsUpdateController extends Controller
{
    ...
    public function handleUpdate(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "HistoricalReports", "action" => "Update"]);
        }

        return Inertia::render("Ivr/HistoricalReports/Update", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2095, "idx" => 58],
        ]);
    }
