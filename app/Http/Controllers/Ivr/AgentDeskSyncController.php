...
class AgentDeskSyncController extends Controller
{
    ...
    public function handleSync(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "AgentDesk", "action" => "Sync"]);
        }

        return Inertia::render("Ivr/AgentDesk/Sync", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2049, "idx" => 27],
        ]);
    }
