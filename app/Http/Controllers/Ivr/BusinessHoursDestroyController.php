...
class BusinessHoursDestroyController extends Controller
{
    ...
    public function handleDestroy(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "BusinessHours", "action" => "Destroy"]);
        }

        return Inertia::render("Ivr/BusinessHours/Destroy", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2066, "idx" => 38],
        ]);
    }
