...
class CallAnalyticsIndexController extends Controller
{
    ...
    public function handleIndex(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(["data" => $rows, "module" => "CallAnalytics", "action" => "Index"]);
        }

        return Inertia::render("Ivr/CallAnalytics/Index", [
            "rows" => $rows,
            "filters" => $request->all(),
            "legacyMeta" => ["seed" => 2083, "idx" => 49],
        ]);
    }
